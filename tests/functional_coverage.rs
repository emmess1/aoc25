...
fn functional_coverage_end_to_end() {
    ...
    for i in 0..8 {
        hm2.insert(Collide(i), i as i32);
    }
    for i in 0..8 {
        assert_eq!(hm2.get(&Collide(i)), Some(&(i as i32)));
    }
    ...
    for k in ["5", "3", "7", "2", "4", "6", "6a"] {
        bst5.insert(k.to_string(), 1);
    }
...
fn functional_coverage_all_ds() {
    ...
    ih.set(0, 5);
    ih.set(0, 1);
    assert_eq!(ih.pop_min().unwrap().0, 0);
    ...
    iset.add(Interval::new(1, 2));
    iset.add(Interval::new(3, 3));
    iset.add(Interval::new(2, 2));
    fcov::hit("interval_set_merge");

    // Monotonic queues
    let mut qmin = MonotonicQueueMin::new();
    qmin.push(3);
    qmin.push(2);
    qmin.pop_if(3);
    fcov::hit("mono_min");
    let mut qmax = MonotonicQueueMax::new();
    qmax.push(1);
    qmax.push(4);
    qmax.pop_if(0);
    fcov::hit("mono_max");
    ...
    // SCC
    let mut adj2 = vec![vec![], vec![0]];
    adj2[0].push(1);
    let _ = tarjan_scc(&adj2);
    fcov::hit("scc_tarjan");
    ...
    g2[0] = vec![1, 2];
    let _ = bfs_distances(n, &g2, 0);
    fcov::hit("bfs_dist");
    let mut gw = vec![vec![]; n];
    gw[0].push((1, 1));
    let (_d, _p) = dijkstra_indexed(n, &gw, 0);
    fcov::hit("dijkstra");
    let h = |_u: usize| 0;
    let _ = astar_indexed(n, &gw, 0, 1, &h);
    fcov::hit("astar");
    ...
    // SparseGrid
    let mut sg: SparseGrid<i32> = SparseGrid::new();
    sg.insert(Point::new(0, 0), 1);
    let _ = sg.bounds();
    ...
    // String algorithms
    let _ = kmp_search("abcabca", "ab");
    fcov::hit("kmp_found");
    let _ = z_function("aaaa");
    fcov::hit("z_func");
    let rh = RollingHash::new("abc", 911382323, 972663749);
    let _ = rh.hash(0, 2);
    fcov::hit("rolling_hash");

    // Topo
    let edges = [(1, 2), (2, 3)];
    let _ = topo_sort(&edges).unwrap();
    fcov::hit("topo_sort_ok");
    let edges2 = [(1, 1)];
    let _ = topo_sort(&edges2);
    fcov::hit("topo_detect_cycle");

    // Parsing
    let _ = parse_grid_chars("ab\n");
    fcov::hit("parse_grid_chars");
    let _ = parse_ints_whitespace("1 -2");
    fcov::hit("parse_ints_ws");
