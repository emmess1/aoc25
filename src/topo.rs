...
pub fn topo_sort<N: Eq + Hash + Clone>(edges: &[(N, N)]) -> Option<Vec<N>> {
    ...
    let mut q: VecDeque<N> = indeg.iter().filter(|(_, &d)| d==0).map(|(n,_)| n.clone()).collect();
    ...
    if out.len() == indeg.len() { Some(out) } else { None }
}

#[derive(Clone, Debug, Default)]
pub struct Topo<N: Eq + Hash + Clone> { edges: Vec<(N,N)> }

impl<N: Eq + Hash + Clone> Topo<N> {
    pub fn new() -> Self { Self { edges: Vec::new() } }
    pub fn add_edge(&mut self, u: N, v: N) { self.edges.push((u,v)); }
    pub fn solve(&self) -> Option<Vec<N>> { topo_sort(&self.edges) }
}
