...
impl Fenwick {
    ...
    pub fn add(&mut self, mut i: usize, delta: i64) { i+=1; while i<=self.n { self.bit[i] += delta; i += i & (!i+1); } }
    /// Prefix sum [0..=i] (0-based). Returns 0 if i is usize::MAX.
    pub fn sum_prefix(&self, mut i: usize) -> i64 { let mut s=0i64; i+=1; while i>0 { s+=self.bit[i]; i &= i-1; } s }
    /// Range sum [l..=r].
    pub fn sum_range(&self, l: usize, r: usize) -> i64 { if r<l {0} else { self.sum_prefix(r) - if l==0 {0} else { self.sum_prefix(l-1) } } }
}
